...
Tip: your method will have to accept a using `JsWriter` to do the actual conversion.

<div class="solution">
```scala mdoc:silent
object JsUtil:
  def toJson[A](value: A)(using writer: JsWriter[A]) =
    writer.write(value)
```
</div>
...
enum Visitor:
  ...
  def age: Long =
    val created = this match
      case a: Anonymous => a.createdAt
      case u: User      => u.createdAt
...
given AnonymousWriter: JsWriter[Anonymous] with
...
given UserWriter: JsWriter[User] with
...
given VisitorWriter: JsWriter[Visitor] with
...
Let's improve our JSON syntax by combining type classes and type enrichment. Add an extension method to all types with a `toJson` method. Sample usage:
...
```scala mdoc:silent
extension [A](value: A)
  def toJson(using writer: JsWriter[A]) =
    writer.write(value)
```
...
```scala mdoc:silent
given StringWriter: JsWriter[String] with
  def write(value: String) = JsString(value)

given DateWriter: JsWriter[Date] with
...
// I must repeat this here for some reason or the given resolution in the block below fails
extension [A](value: A)
  def toJson(using writer: JsWriter[A]) =
    writer.write(value)
```
