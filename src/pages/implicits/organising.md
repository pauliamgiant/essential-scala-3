...
In this section we'll learn about the places the compiler searches for type class instances (given values), known as the *given scope*, and we'll discuss how to organise type class instances to make their use more convenient.

### Given Scope

The compiler searches the given scope when it tries to find a given value to supply as a using parameter. The given scope is composed of several parts, and there are rules that prioritise some parts over others.

The first part of the given scope is the normal scope where other identifiers are found. This includes identifiers declared in the local scope, within any enclosing class, object, or trait, or `import`ed from elsewhere. An eligible given value must be a single identifier (i.e. `a`, not `a.b`). This is referred to as the *local scope*.

The given scope also includes the companion objects of types involved in the method call with the using parameter. Let's look at `sorted` for example. The signature for `sorted`, defined on `List[A]`, is

```scala
def sorted[B >: A](using ord: math.Ordering[B]): List[A]
```
...
Here we get the error above at compilation time.
...
To understand this we need to learn about the priority rules for selecting givens. An ambiguity error is only raised if there are multiple type class instances with the same priority. Otherwise the highest priority given is selected.

The practical implication is that the local scope takes precedence over instances found in companion objects. This means that givens that the programmer explicitly pulls into scope, by importing or defining them in the local scope, will be used in preference.
...
```scala mdoc:silent
object Example:
  given higherPriorityGiven: Ordering[Rational] =
    Ordering.fromLessThan[Rational]((x, y) =>
      (x.numerator.toDouble / x.denominator.toDouble) >
        (y.numerator.toDouble / y.denominator.toDouble),
    )
...
Notice that `higherPriorityGiven` defines a different ordering to the one defined in the companion object for `Rational`. We've also changed the expected ordering in `example` to match this new ordering. This code both compiles and runs correctly, illustrating the effect of the priority rules.
...
### Packaging Given Values Without Companion Objects
...
In use the user would `import RationalLessThanOrdering.given` or `import RationalGreaterThanOrdering.given` as appropriate.

### Take Home Points

The compiler looks for type class instances (given values) in two places:

1. the local scope; and
2. the companion objects of types involved in the method call.

Givens found in the local scope take precedence over those found in companion objects.

When packaging type class instances, if there is a single instance or a single good default we should put it in the companion object if possible. Otherwise, one way to package givens is to place each one in an object and require the user to explicitly import them.
...
Implement and package givens to provide these orderings, and justify your packaging.
